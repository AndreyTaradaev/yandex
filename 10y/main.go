...
func main() {
	var in string  // строка, состоящая из строчных латинских букв — любимое слово Лёши.
//Длина строки лежит в пределах от 5 до 100 000 символов.
	...
	_, err = fmt.Fscan(rd, &in) //   параметры матриц и количество запросов
	if err != nil {
		fmt.Println(err)
		return
	}	
 ...
 //fmt.Println(sb)
length := len(sb)
slout := make ([]int,int('z'),int('z'))
 for i := 0; i < length; i++ {	
	slout[sb[i]-byte('a')]+= (i+1)*(length-i) 
	
 }
// fmt.Println(slout)
 for  key, val := range slout {
	if(val >0){
	w := key+int('a')			
	fmt.Println( string(w)+":",val)
	}	
} 
